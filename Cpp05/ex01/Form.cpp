...
Form::Form(): _name("default"), _sign_grade(150), _execute_grade(150), _signed(false){
    std::cout<<"Form -"<<this->_name<<"- signing min. grade: "<<this->_sign_grade<<" executing min. grade: "<<this->_execute_grade<<std::endl;
};

Form::Form(const std::string name): _name(name), _sign_grade(150), _execute_grade(150), _signed(false){
    std::cout<<"Form Constructor -"<<this->_name<<"- signing min. grade: "<<this->_sign_grade<<" executing min. grade: "<<this->_execute_grade<<std::endl;
};

Form::Form(const std::string name, const int sign_grade, const int exec_grade): _name(name), _sign_grade(sign_grade), _execute_grade(exec_grade), _signed(false){
    std::cout<<"Form Constructor -"<<this->_name<<"- signing min. grade: "<<this->_sign_grade<<" executing min. grade: "<<this->_execute_grade<<std::endl;
};

Form::Form(const Form &copy): _name(copy._name + ".copy"), _sign_grade(copy._sign_grade), _execute_grade(copy._execute_grade), _signed(copy._signed){
